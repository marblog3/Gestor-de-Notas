...
// --- Cargar datos personales del alumno (MIGRADO A DB) ---
async function cargarDatosPersonales() {
    ...
    if (tabla) {
        ...
        // Cargar información del curso/especialidad
        try {
            const response = await fetch(`../api/get_user_by_email.php?email=${activeUser.email}`);
            const data = await response.json();
            
            if (data.success && data.user.curso_info) {
                const curso = JSON.parse(data.user.curso_info).curso;
                if (curso) {
                    tabla.querySelector("td:nth-child(5) input").value = curso.anio || '';
                    tabla.querySelector("td:nth-child(6) input").value = curso.division || '';
                    tabla.querySelector("td:nth-child(4) input").value = curso.especialidad || '';
                    // Nota: Turno y Preceptor no están en la estructura actual de curso_info.
                }
            }
        } catch (e) {
